...
const actions = {
  ...
  feathersUpdateQueries({ get, set }, { serviceName, entity, method }) {
    ...
    forEachObj(queries, (query, key) => {
      ...
      const matches = method === 'remove' ? false : matcher(query.params.query)(entity)
    ...
    if (updateCount > 0) {
      ...
      if (method === 'create' && !getIn(next, ['entities', serviceName, entity.id])) {
        next = setIn(next, ['entities', serviceName, entity.id], entity)
      }
