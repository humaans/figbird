...
const actions = ({ idField, updatedAtField }) => ({
  feathersFetched: updater('fetch', idField, updatedAtField),

  feathersCreated: updater('create', idField, updatedAtField),

  feathersUpdated: updater('update', idField, updatedAtField),

  feathersPatched: updater('patch', idField, updatedAtField),

  feathersRemoved: remover(idField, updatedAtField),

  feathersUpdateQueries(
    { get, set },
    { serviceName, entity, method, additionalFilters, additionalOperators }
  ) {
    ...
    if (updateCount > 0) {
      ...
      if (method === 'create' && !getIn(next, ['entities', 'live', serviceName, idField(entity)])) {
        next = setIn(next, ['entities', 'live', serviceName, idField(entity)], entity)
      }
...
function updater(method, idField, updatedAtField) {
  return (
    { get, set, actions },
    {
      serviceName,
    ...
    if (entity && method !== 'create') {
      // check if we should discard this update
      const currEntity = getIn(curr, ['entities', entitiesKey, serviceName, idField(entity)])
      if (currEntity) {
        const currUpdatedAt = updatedAtField(currEntity)
        const nextUpdatedAt = updatedAtField(entity)
        if (currUpdatedAt && nextUpdatedAt && nextUpdatedAt < currUpdatedAt) {
          return
        }
      }

      next = setIn(curr, ['entities', entitiesKey, serviceName, idField(entity)], entity)
    ...
    if (data) {
      ...
      const entities = { ...getIn(curr, ['entities', entitiesKey, serviceName]) }
      for (const entity of items) {
        entities[idField(entity)] = entity
      }

      // update entities
      next = setIn(next, ['entities', entitiesKey, serviceName], entities)
...
function remover(idField) {
  return ({ get, set, actions }, { serviceName, entity }) => {
    ...
    if (getIn(curr, ['entities', 'live', serviceName, idField(entity)])) {
      ...
      const serviceEntities = { ...getIn(curr, ['entities', 'live', serviceName]) }
      delete serviceEntities[idField(entity)]
      next = setIn(next, ['entities', 'live', serviceName], serviceEntities)
