...
class QueryStore {
  ...
  constructor({ feathers, idField = defaultIdField, updatedAtField = defaultUpdatedAtField }) {
  ...
  subscribe(queryId, fn) {
    ...
    return ({ remove = shouldRemoveByDefault } = {}) => {
      removeListener()
      if (remove && this.#listenerCount(queryId) === 0) {
        this.#vacuum({ queryId })
      }
    }
  }
    ...
    if (query) {
      if (query.state.data) {
        ...
        for (const item of items) {
          next = updateIn(next, ['index', query.desc.serviceName, this.#getId(item)], index => {
  ...
  #computeFetched(curr, { queryId, result }) {
    ...
    // if (data) {
    //   next = this.#updateQueries(next, { serviceName, method: 'find', item: data })
    // }
  ...
  #updateQueries(curr, { serviceName, method, item: itemOrItems }) {
    ...
    for (const item of items) {
      ...
      // First update the entity in the entities cache
      next = setIn(next, ['entities', serviceName, itemId], item)
      ...
      for (const [queryId, query] of Object.entries(queries)) {
        ...
        // filter for the right queries
        if (query.desc.serviceName !== serviceName) {
          break
        }

        // If this query has this item indexed, we should update it
        // regardless of current matching status
        const hasItem = index.queries[queryId]

        if (method === 'remove') {
          matches = false
        } else {
          const matcher = query.config.matcher
            ? query.config.matcher(defaultMatcher)
            : defaultMatcher
          matches = matcher(query.desc.params.query)(item)
        }
...
export class Figbird {
  ...
  // sub to state changes, useful if you want
  // to do something with the entire queryStore state
  subscribeToStateChanges(fn) {
