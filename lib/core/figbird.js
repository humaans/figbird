...
class QueryStore {
  ...
  #getQuery(queryId) {
    ...
    if (serviceName) {
      const service = this.getState()[serviceName]
      if (service) {
  ...
  #setState(fn, { silent = false } = {}) {
    ...
    const wrappedFn = state => fn(state, touch)

    this.#state = wrappedFn(this.#state)
  ...
  #transactOverServiceByName(serviceName, fn, { silent = false } = {}) {
    if (serviceName) {
      ...
      this.#setState(
        (state, touch) => {
          fn(state[serviceName], touch)
          // TODO - improve, for now we simulate a change, even though we are mutating the state directly now
          return { ...state }
        },
        { silent },
      )
  ...
  #updated({ serviceName, item }) {
    ...
    const currItem = this.getState()[serviceName].entities.get(itemId)
