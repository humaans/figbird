...
class QueryStore {
  ...
  #state = {} // State is now keyed by serviceName at the top level
  ...
  #getQuery(queryId) {
    ...
    return serviceName ? getIn(this.getState(), [serviceName, 'queries', queryId]) : undefined
  }
  ...
  materialize(queryRef) {
    this.#setState(
      state => {
        ...
        if (getIn(state, [serviceName, 'queries', queryId])) {
          return state
        } else {
          let next = state
          if (!next[serviceName]) {
            next = setIn(next, [serviceName], {
              entities: {},
              queries: {},
              itemQueryIndex: {},
            })
          }
          if (!next[serviceName].queries) {
            next = setIn(next, [serviceName, 'queries'], {})
          }
          this.#serviceNamesByQueryId.set(queryId, serviceName)
          return setIn(next, [serviceName, 'queries', queryId], {
            queryId,
            desc,
            config,
            pending: true,
  ...
  #refetchRefetchableQueries() {
    const state = this.getState()
    for (const serviceName of Object.keys(state)) {
      const serviceQueries = state[serviceName].queries
  ...
  #setState(fn, { silent = false } = {}) {
    ...
    if (!silent && prev !== this.#state) {
      for (const serviceName of Object.keys(this.#state)) {
        const serviceQueries = this.#state[serviceName].queries
        for (const [queryId, nextQuery] of Object.entries(serviceQueries)) {
          const prevQuery = getIn(prev, [serviceName, 'queries', queryId])
  ...
  #computeVacuum(curr, { queryId }) {
    ...
    const serviceName = this.#serviceNamesByQueryId.get(queryId)
    const query = getIn(next, [serviceName, 'queries', queryId])
    ...
    if (query) {
      if (query.state.data) {
        ...
        for (const item of items) {
          next = updateIn(
            next,
            [serviceName, 'itemQueryIndex', this.#adapter.getId(item)],
            index => {
              const next = { ...index }
              delete next[queryId]
              return next
      ...
      next = unsetIn(next, [serviceName, 'queries', queryId])
  ...
  #computeFetching(curr, { queryId }) {
    ...
    if (getIn(next, [serviceName, 'queries', queryId])) {
      next = setIn(next, [serviceName, 'queries', queryId, 'pending'], false)
      next = updateIn(next, [serviceName, 'queries', queryId, 'state'], state =>
        state.status === 'error'
  ...
  #computeFetched(curr, { queryId, result }) {
    ...
    const entities = { ...getIn(next, [serviceName, 'entities']) }
    const itemQueryIndex = { ...getIn(next, [serviceName, 'itemQueryIndex']) }
    ...
    next = setIn(next, [serviceName, 'entities'], entities)
    next = setIn(next, [serviceName, 'itemQueryIndex'], itemQueryIndex)

    next = setIn(next, [serviceName, 'queries', queryId, 'state'], {
      data,
      meta,
      status: 'success',
      isFetching: false,
  ...
  #computeUpdated(curr, { serviceName, item }) {
    ...
    const currItem = getIn(curr, [serviceName, 'entities', itemId])
    ...
    let next = curr
    if (currItem) {
      next = setIn(next, [serviceName, 'entities', itemId], item)
    } else {
      next = setIn(next, [serviceName, 'entities', itemId], item)
      next = setIn(next, [serviceName, 'itemQueryIndex', itemId], {})
    }
  ...
  #computeRemoved(curr, { serviceName, item: itemOrItems }) {
    ...
    const exists = items.some(item =>
      getIn(curr, [serviceName, 'entities', this.#adapter.getId(item)]),
    )
    ...
    const serviceEntities = { ...getIn(next, [serviceName, 'entities']) }
    for (const item of items) {
      delete serviceEntities[this.#adapter.getId(item)]
      next = setIn(next, [serviceName, 'entities'], serviceEntities)
    }
    return next
  }
  ...
  #updateQueries(curr, { serviceName, method, item: itemOrItems }) {
    ...
    for (const item of items) {
      const itemId = this.#adapter.getId(item)
      const queries = getIn(next, [serviceName, 'queries']) || {}
      const itemQueryIndex = { ...getIn(next, [serviceName, 'itemQueryIndex', itemId]) }
      ...
      if (updateCount > 0) {
        next = setIn(next, [serviceName, 'itemQueryIndex', itemId], itemQueryIndex)
        if (Object.keys(itemQueryIndex).length === 0) {
          next = unsetIn(next, [serviceName, 'entities', itemId])
          next = unsetIn(next, [serviceName, 'itemQueryIndex', itemId])
        }
      }
    }
