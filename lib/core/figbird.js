...
class QueryStore {
  ...
  mutate({ serviceName, method, args }) {
    const updaters = {
      create: item => this.#queueEvent(serviceName, { type: 'created', item }),
      update: item => this.#queueEvent(serviceName, { type: 'updated', item }),
      patch: item => this.#queueEvent(serviceName, { type: 'patched', item }),
      remove: item => this.#queueEvent(serviceName, { type: 'removed', item }),
    }
