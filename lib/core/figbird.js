...
export class Figbird {
  ...
  watchQuery(query) {
    ...
    // TODO - simplificate
    const { status, isFetching } = this.getQueryState(query.queryId)
    if (
      !existed ||
      (status !== 'success' && status !== 'loading') ||
      (status === 'success' && query.fetchPolicy === 'swr' && !isFetching) ||
      (status === 'success' && query.fetchPolicy === 'network-only' && !isFetching)
    ) {
      this.queue(queryId)
    }
  ...
  fetch(queryId) {
    ...
    return fetch(this.feathers, query.serviceName, query.method, query.resourceId, query.params, {
  ...
  #computeFetched(curr, { queryId, result }) {
    const query = getIn(curr, ['queries', queryId])
    const { serviceName, realtime } = query
    ...
    if (result && query.method === 'find' && typeof result === 'object' && result.data) {
      items = result.data
      data = items
      meta = { ...result }
      delete meta.data
    ...
    for (const item of items) {
      ...
      forEachObj(queries, (query, queryId) => {
        ...
        if (method === 'remove') {
          ...
          const match = query.matcher ? query.matcher(defaultMatcher) : defaultMatcher
          matches = match(query.params.query)(item)
        }
