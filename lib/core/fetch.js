export function fetch(
  ...
  resourceId,
  params,
  { queryId, allPages, parallel, parallelLimit = 4 },
) {
  // TODO - argh, params vs query?? move all to params, move query to query, but then what is the top level ffs
  const service = feathers.service(serviceName)
  const result =
    method === 'get'
      ? get(service, resourceId, params, { queryId })
      : find(service, params, {
          queryId,
          allPages,
          parallel,
