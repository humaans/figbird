...
export class QueryCache {
  ...
  #computeFetching(curr, { serviceName, data, method, params, queryId, realtime, matcher }) {
    let next = curr
  ...
  #computeFetched(curr, { serviceName, data, method, params, queryId, realtime, matcher }) {
  ...
  #computeCreated(curr, { serviceName, item }, config) {
    return this.#updateQueries(curr, { serviceName, method: 'create', item }, config)
  }
  ...
  #computePatched(curr, payload, config) {
    return this.#computeUpdated(curr, payload, config)
  }
      ...
      forEachObj(queries, (query, queryId) => {
        ...
        if (index.queries[queryId]) {
          if (!matches) {
            ...
            queries[queryId] = {
              ...query,
              data: query.data.map(x => (this.getId(x) === itemId ? item : x)),
            }
          }
        } else {
