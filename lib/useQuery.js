    ...
    fetch(feathers, serviceName, method, id, params, {
      ...
      .then(res => {
        if (reqRef === requestRef.current) {
          log({ queryId, serviceName, method, id, params, debug }, 'success', res)
          flushSync(() => {
            updateCache(res)
      ...
      .catch(error => {
        if (reqRef === requestRef.current) {
          log({ queryId, serviceName, method, id, params, debug }, 'success', error)
          dispatch({ type: 'error', error })
        }
      })
      .finally(() => {
        if (reqRef !== requestRef.current) {
          log({ queryId, serviceName, method, id, params, debug }, 'superseded')
        }
      })
  }, [
    feathers,
...
function log({ queryId, serviceName, method, id, debug }, msg, ctx) {
  if (debug) {
    console.log(
      `✨ [${queryId}] Fetching ${serviceName}#${method}${id ? ` ${id}` : ''}${msg ? ` - ${msg}` : ''}`,
      ctx,
    )
  }
}
