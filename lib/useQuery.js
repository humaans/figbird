...
function reducer(state, action) {
  switch (action.type) {
    case 'success':
      return { ...state, status: 'success', dirty: false }
    case 'error':
      return { ...state, status: 'error', error: action.error, dirty: false }
    case 'reset':
      return {
        ...state,
        status: 'pending',
        error: null,
        dirty: action.dirty ?? (state.dirty || state.status === 'pending'),
      }
  }
}
...
export function useQuery(serviceName, options = {}, queryHookOptions = {}) {
  ...
  useEffect(() => {
    // nothing happens while skipping
    if (skip) return

    // already fetching at the moment
    if (!isPending) return
    ...
    fetch(feathers, serviceName, method, id, params, {
      ...
      .then(res => {
        if (state.dirty) {
          dispatch({ type: 'reset', dirty: false })
  ...
  // refetch if queryId changed (which means the query changed),
  // but not if this is the first render
  const previousQueryIdRef = useRef(null)
  useEffect(() => {
    if (
    ...
    previousQueryIdRef.current = queryId
  }, [dispatch, queryId, isCacheSufficient])
