  ...
  return new Promise((resolve, reject) => {
    ...
    function fetchNext() {
      if (
        ...
        const requiredFetches = (result.total / result.limit) | 0

        if (requiredFetches > 0) {
          Promise.all(
            new Array(requiredFetches).fill().map((_, idx) => {
              return service.find({
                ...params,
                query: {
                  ...(params.query || {}),
                  $skip: (idx + 1) * result.limit,
                },
              })
            })
          )
            .then(results => {
              ...
              if (lastResult.data.length === 0 || result.data.length >= lastResult.total) {
                resolve(result)
              } else {
                skip = lastResult.data.length
                fetchNext()
              }
              resolve()
            })
