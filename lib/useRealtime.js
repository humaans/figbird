...
export function useRealtime(serviceName, mode, cb) {
  ...
  useEffect(() => {
    ...
    if (ref.realtime === 1) {
      ref.created = entity => {
        actions.feathersCreated({ serviceName, entity })
        refs[serviceName].callbacks.forEach(c => c({ event: 'created', serviceName, entity }))
      }
      ref.updated = entity => {
        actions.feathersUpdated({ serviceName, entity })
        refs[serviceName].callbacks.forEach(c => c({ event: 'updated', serviceName, entity }))
      }
      ref.patched = entity => {
        actions.feathersPatched({ serviceName, entity })
        refs[serviceName].callbacks.forEach(c => c({ event: 'patched', serviceName, entity }))
      }
      ref.removed = entity => {
        actions.feathersRemoved({ serviceName, entity })
        refs[serviceName].callbacks.forEach(c => c({ event: 'removed', serviceName, entity }))
      }
