  ...
  return new Promise((resolve, reject) => {
    ...
    function resolveOrFetchNext(res) {
      if (
        ...
        (isTotalAvailale(res) && result.data.length >= res.total)
      ) {
        resolve(result)
      } else {
    ...
    function fetchNextParallel() {
      ...
      const requiredFetches = isTotalAvailale(result)
        ? Math.min(Math.ceil((result.total - result.data.length) / result.limit), parallelLimit)
        : optimisticParallelLimit
