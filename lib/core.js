...
export const Provider = ({ feathers, children, ...props }) => {
  ...
  const idFieldGetter = props.idField || defaultIdField
  const idField = useCallback(
    item => {
      const id = typeof idFieldGetter === 'string' ? item[idField] : idFieldGetter(item)
  ...
  const updatedAtFieldGetter = props.updatedAtField || defaultUpdatedAtField
  const updatedAtField = useCallback(
    item => {
      const updatedAt =
        typeof updatedAtFieldGetter === 'string'
          ? item[updatedAtFieldGetter]
          : updatedAtFieldGetter(item)
      return updatedAt
    },
    [updatedAtFieldGetter]
  )
