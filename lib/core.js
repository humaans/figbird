...
export const Provider = ({ feathers, children, ...props }) => {
  ...
  const { atom, AtomProvider, useSelector } = useCacheInstance(atomFromProps || atomFromContext)
  ...
  const figbird = useMemo(() => {
    return { feathers, atom, actions: atom.actions, useSelector }
  }, [feathers, atom, useSelector])
